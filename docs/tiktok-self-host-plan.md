# Self-hosting davidteather/TikTok-Api for WhatTheFad?
...
- Cold start is fine for WhatTheFad?'s use case: user opens the TikTok tab, we show `loading…`, fetches resolve in under a minute
...
- Second failure mode: Render free tier spins down. First request after idle is 30–60s. WhatTheFad?'s loading spinner handles it, but consider: if this becomes annoying, either (a) upgrade Render to $7/mo starter (always-on), or (b) add a cron that pings `/health` every 10 min to keep it warm.
...
- WhatTheFad? wiring + deploy: 10 min
